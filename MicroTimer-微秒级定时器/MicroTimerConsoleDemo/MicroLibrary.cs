...
//•MicroStopwatch - This derives from and extends the System.Diagnostics.Stopwatch class;
//importantly, it provides the additional property ElapsedMicroseconds.
//This is useful as a standalone class where the elapsed microseconds from when the stopwatch was started can be directly obtained.
//•MicroTimer - Designed so it operates in a very similar way to the System.Timers.Timer class, 
...
//By design, the amount of work done in the callback function (OnTimedEvent) must be small (e.g. update a variable or fire off a UDP packet).
//To that end, the work done in the callback function must take significantly less time than the timer interval. Separate threads could be spawned for longer tasks;
//    however, this goes outside the scope of this article. As discussed earlier, because Windows is not a real time Operating System, the callback function (OnTimedEvent) may be late; if this happens and any particular interval is delayed, there are two options:
//•Either: Set the property IgnoreEventIfLateBy whereby the callback function (OnTimedEvent) will not be called if the timer is late by the specified number of microseconds.
//    The advantage of this is the timer will not attempt to 'catch up', i.e., it will not call the callback function in quick succession in an attempt to catch up. 
//        The disadvantage is that some events will be missed.
//•Or: By default, MicroTimer will always try and catch up on the next interval.
//    The advantage of this is the number of times the OnTimeEvent is called will always be correct for the total elapsed time (which is why the OnTimedEvent must take significantly less time than the interval; if it takes a similar or longer time, MicroTimer can never 'catch up' and the timer event will always be late). The disadvantage of this is when it's trying to 'catch up', the actual interval achieved will be much less than the required interval as the callback function is called in quick succession in an attempt to catch up.
