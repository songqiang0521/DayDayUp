    ...
    if (QueryDosDevice(lpDrive, lpDeviceName, 0x1000) != 0)
    {
        ...
        if (_tcsnicmp(_T("\\??\\"), lpDeviceName, 4) == 0)
        {
            ...
            if (_tcsnicmp(_T("\\Device\\LanmanRedirector\\"), lpDeviceName, 25) == 0)
            {
                ...
                if (_tscanf_s/*_stscanf*/(lpDeviceName,
                    _T("\\Device\\LanmanRedirector\\;%c:%d\\%s"),
                    &cDriveLetter,
                    &dwParam,
                    lpSharedName) != 3)
...
BOOL SystemProcessInformation::Refresh()
{
    ...
    if (INtDll::NtQuerySystemInformation(5, m_pBuffer, BufferSize, &realLength) != 0)
    {
        // The size was not enough
        VirtualFree(m_pBuffer, 0, MEM_RELEASE);

        DWORD newLength = realLength + 1024;
        m_pBuffer = (UCHAR*)VirtualAlloc(NULL, newLength, MEM_COMMIT, PAGE_READWRITE);
        if (m_pBuffer == NULL)
        {
            return FALSE;
        }

        if (INtDll::NtQuerySystemInformation(5, m_pBuffer, newLength, &newLength) != 0)
        {
            return FALSE;
        }
...
HANDLE SystemHandleInformation::DuplicateHandle(HANDLE hProcess, HANDLE hRemote)
{
    ...
    BOOL ret = ::DuplicateHandle(hProcess, hRemote, GetCurrentProcess(), &hDup, 0, FALSE, DUPLICATE_SAME_ACCESS);
    if (ret == FALSE)
    {
        DWORD error = GetLastError();
        int xx = error;
