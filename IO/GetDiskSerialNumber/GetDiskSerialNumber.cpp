    ...
    for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
    {
        ...
        else
        {
                ...
                &cbBytesReturned, NULL))
            {
                ...
                EflipAndCodeBytes(local_buffer, descrip->VendorIdOffset, 0, vendorId);
                printf("venderID: %s\n", vendorId);

                EflipAndCodeBytes(local_buffer, descrip->ProductIdOffset, 0, modelNumber);
                printf("modelNumber: %s\n", modelNumber);

                EflipAndCodeBytes(local_buffer, descrip->ProductRevisionOffset, 0, productRevision);
                printf("productRevision: %s\n", productRevision);

                EflipAndCodeBytes(local_buffer, descrip->SerialNumberOffset, 1, serialNumber);
